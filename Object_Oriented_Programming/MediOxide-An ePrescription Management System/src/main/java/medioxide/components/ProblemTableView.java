...
public class ProblemTableView<T> extends TableView<ProblemTableViewModel> {

    OnClickListener listener;

    public ProblemTableView(ObservableList<ProblemTableViewModel> observableList, OnClickListener listener) {
    ...
    private void rowSetup() {
        setRowFactory(tTableView -> {
            TableRow<ProblemTableViewModel> row = new TableRow<ProblemTableViewModel>() {
                @Override
                protected void updateItem(ProblemTableViewModel t, boolean b) {
    ...
    private void columnSetup() {
        var column1 = new TableColumn<ProblemTableViewModel, Integer>("ID");
        column1.setPrefWidth(20);
        column1.setCellValueFactory(cellData -> cellData.getValue().idProperty().asObject());

        var column2 = new TableColumn<ProblemTableViewModel, String>("Name");
        column2.setPrefWidth(30);
        column2.setCellValueFactory(cellData -> cellData.getValue().nameProperty());

        var column3 = new TableColumn<ProblemTableViewModel, String>("Dept");
        column3.setPrefWidth(40);
        column3.setCellValueFactory(cellData -> cellData.getValue().departmentProperty());


        var column4 = new TableColumn<ProblemTableViewModel, String>("Description");
        column4.setPrefWidth(40);
        column4.setCellValueFactory(cellData -> cellData.getValue().descriptionProperty());

        var column5 = new TableColumn<ProblemTableViewModel, String>("Symptoms");
        column5.setPrefWidth(40);
        column5.setCellValueFactory(cellData -> cellData.getValue().symptomsProperty());

        var column6 = new TableColumn<ProblemTableViewModel, String>("Treatment");
        column6.setPrefWidth(40);
        column6.setCellValueFactory(cellData -> cellData.getValue().treatmentProperty());


        var column7 = new TableColumn<ProblemTableViewModel, Boolean>("Action");
        ...
        ObservableList<TableColumn<ProblemTableViewModel, ?>> tableHeaders = FXCollections.observableArrayList(
                column1,
                column2,
                column3,
                column4,
    ...
    private class ButtonCell<S, T> extends TableCell<S, T> {
        ...
        ButtonCell() {
            ...
            editButton.setOnMouseClicked(mouseEvent -> {
                ...
                if (currentItem instanceof ProblemTableViewModel m) {
